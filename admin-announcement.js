// ============================================================
// admin-announcement.js - Announcement 管理
// ============================================================
...
(function() {
    ...
    } catch (e) { console.warn('⚠️ Supabase 不可用:', e); }
    ...
    function renderAnnouncementPage() {
                ...
                itemsHTML += `
                    <div class="announcement-item" style="margin-bottom:8px;">
                        <div style="flex:1;min-width:0;">
                            <div style="font-weight:600;color:#e5e9f0;font-size:0.9rem;">${escapeHtml(item.text || 'Untitled')}</div>
                            <div style="display:flex;align-items:center;gap:12px;margin-top:2px;">
                                <span class="badge-status ${statusClass}">${statusText}</span>
                                <span style="font-size:0.65rem;color:#4a5a7a;"><i class="far fa-clock"></i> ${item.time || 'Today'}</span>
                                ${item.link ? `<span style="font-size:0.65rem;color:#2a3560;"><i class="fas fa-link"></i> ${escapeHtml(item.link)}</span>` : ''}
                            </div>
                        </div>
                        <div style="display:flex;gap:6px;flex-shrink:0;">
                            <button onclick="window._adminAnnouncement.openEditModal('${item.id}')" style="background:rgba(74,124,255,0.1);border:1px solid rgba(74,124,255,0.15);color:#4a7cff;padding:5px 12px;border-radius:6px;cursor:pointer;font-size:0.65rem;font-weight:600;"><i class="fas fa-edit"></i> Edit</button>
                            <button onclick="window._adminAnnouncement.deleteItem('${item.id}')" style="background:rgba(255,107,107,0.08);border:1px solid rgba(255,107,107,0.1);color:#ff6b6b;padding:5px 12px;border-radius:6px;cursor:pointer;font-size:0.65rem;font-weight:600;"><i class="fas fa-trash"></i> Delete</button>
                            <button onclick="window._adminAnnouncement.toggleStatus('${item.id}')" style="background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.05);color:#5a6388;padding:5px 10px;border-radius:6px;cursor:pointer;font-size:0.65rem;">
                                <i class="fas fa-${item.status === 'active' ? 'eye' : 'eye-slash'}"></i> ${item.status === 'active' ? 'Hide' : 'Show'}
                            </button>
                        </div>
                    </div>
                `;
        ...
        var html = `
            <div style="margin-bottom:16px;display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:12px;">
                <div>
                    <div style="font-size:18px;font-weight:600;color:#fff;"><i class="fas fa-bullhorn" style="color:#4dc8ff;"></i> Announcements</div>
                    <div style="font-size:12px;color:#5a6388;">Manage homepage scrolling announcements</div>
                </div>
                <button onclick="window._adminAnnouncement.openAddModal()" style="background:linear-gradient(135deg,#4a7cff,#2f3e7a);border:none;padding:10px 24px;border-radius:40px;color:#fff;font-weight:600;font-size:13px;cursor:pointer;display:flex;align-items:center;gap:8px;">
    ...
    function syncFromSupabase() {
        ...
        sb.from('announcements').select('*').order('created_at', { ascending: false })
            ...
            .catch(function(err) { console.warn('⚠️ Supabase 同步异常:', err.message); });
    }
    ...
    function saveData(callback) {
        ...
        if (sb) {
            if (currentItems.length === 0) {
                sb.from('announcements').delete().neq('id', '').then(function() { if (callback) callback(); }).catch(function() { if (callback) callback(); });
                return;
            }
            var promises = [];
            currentItems.forEach(function(item) {
                promises.push(sb.from('announcements').upsert(item, { onConflict: 'id' }).then(function(res) {
                    if (res.error) console.warn('⚠️ 单条同步失败:', res.error.message);
                    return res;
                }));
            });
            Promise.all(promises).then(function() { console.log('✅ 已同步到 Supabase'); if (callback) callback(); })
                .catch(function(err) { console.warn('⚠️ 同步 Supabase 失败:', err.message); if (callback) callback(); });
        } else {
    ...
    function addItem(data) {
        ...
        saveData(function() { showSaveResult(true, 'Announcement added successfully!'); });
    }
    ...
    function updateItem(id, data) {
        ...
        saveData(function() { showSaveResult(true, 'Announcement updated successfully!'); });
    }

    function deleteItem(id) {
        if (!confirm('Delete this announcement?')) return;
        currentItems = currentItems.filter(function(item) { return item.id !== id; });
        saveData(function() { showSaveResult(true, 'Announcement deleted successfully!'); });
    }
    ...
    function showSaveResult(success, message) {
        ...
        if (firstChild) container.insertBefore(div, firstChild);
    ...
    function openFormModal(title, subtitle, item) {
        var isEdit = !!item;
        var bodyHTML = `
            <div class="form-group"><label><i class="fas fa-text"></i> Announcement Text *</label><textarea id="formText" placeholder="Enter announcement text...">${isEdit ? escapeHtml(item.text || '') : ''}</textarea></div>
            <div class="form-group"><label><i class="far fa-clock"></i> Time Tag</label><input type="text" id="formTime" placeholder="e.g. 10:30, Today, Yesterday" value="${isEdit ? escapeHtml(item.time || '') : 'Today'}"></div>
            <div class="form-group"><label><i class="fas fa-tag"></i> Badge</label>
                <select id="formBadge">
            ...
            <div class="form-group"><label><i class="fas fa-link"></i> Link (optional)</label><input type="text" id="formLink" placeholder="e.g. /deposit, https://example.com" value="${isEdit ? escapeHtml(item.link || '') : ''}"></div>
    ...
    console.log('✅ admin-announcement.js loaded');
    loadData();
})();