...
(function() {
    ...
    function renderCarouselPage() {
        ...
        if (items.length === 0) {
            ...
            items.forEach(function(item, index) {
                ...
                itemsHTML += `
                    <div style="display:flex;align-items:center;gap:16px;padding:12px 16px;background:rgba(255,255,255,0.02);border-radius:12px;border:1px solid rgba(255,255,255,0.04);">
                        <div style="width:28px;text-align:center;color:#4a5a7a;font-size:0.7rem;font-weight:600;flex-shrink:0;">#${item.order || index + 1}</div>
                        ${imageDisplay}
                        <div style="flex:1;min-width:0;">
                            ...
                            <div style="color:#5a6388;font-size:0.7rem;margin-top:2px;"><i class="fas fa-link"></i> ${item.cta_link || 'No link'}</div>
                        </div>
                        <div style="display:flex;gap:6px;flex-shrink:0;">
                            <button onclick="window._adminCarousel.openEditModal('${item.id}')" style="background:rgba(74,124,255,0.1);border:1px solid rgba(74,124,255,0.15);color:#4a7cff;padding:5px 12px;border-radius:6px;cursor:pointer;font-size:0.65rem;font-weight:600;display:flex;align-items:center;gap:4px;"><i class="fas fa-edit"></i> Edit</button>
                            <button onclick="window._adminCarousel.deleteItem('${item.id}')" style="background:rgba(255,107,107,0.08);border:1px solid rgba(255,107,107,0.1);color:#ff6b6b;padding:5px 12px;border-radius:6px;cursor:pointer;font-size:0.65rem;font-weight:600;display:flex;align-items:center;gap:4px;"><i class="fas fa-trash"></i> Delete</button>
                            <button onclick="window._adminCarousel.toggleActive('${item.id}')" style="background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.05);color:#5a6388;padding:5px 10px;border-radius:6px;cursor:pointer;font-size:0.65rem;display:flex;align-items:center;gap:4px;">
        ...
        try {
            ...
            if (currentItems.length === 0) {
                sb.from('carousel_items').delete().neq('id', '')
                    .then(function() { console.log('✅ 所有数据已删除'); if (callback) callback(); })
                    .catch(function() { if (callback) callback(); });
                return;
            }
            sb.from('carousel_items').upsert(currentItems, { onConflict: 'id' })
                .then(function(res) { console.log('✅ 保存成功 (upsert):', res); if (callback) callback(); })
                .catch(function(err) {
    ...
    function openAddModal() { editingItemId = null; selectedFile = null; uploadedImageUrl = ''; openFormModal('Add Slide', 'Create a new carousel slide', null); }
    ...
    function openFormModal(title, subtitle, item) {
        var isEdit = !!item;
        var imagePreviewContent = isEdit && item.image_url && item.image_url.startsWith('http')
            ? `<img src="${item.image_url}" style="width:100%;height:100%;object-fit:cover;border-radius:8px;">`
            : `<i class="fas fa-image" style="font-size:2rem;color:#4a5a7a;"></i>`;

        var bodyHTML = `
            <div class="form-group">
                <label style="display:block;font-size:12px;color:#5a6388;margin-bottom:4px;font-weight:600;"><i class="fas fa-image"></i> Slide Image (1600 x 900px recommended)</label>
                <div style="display:flex;align-items:center;gap:12px;background:rgba(10,14,26,0.8);border:1px dashed #2a3560;border-radius:12px;padding:16px;cursor:pointer;" id="uploadArea">
                    <div style="width:80px;height:45px;border-radius:8px;background:rgba(0,180,255,0.05);display:flex;align-items:center;justify-content:center;font-size:1.5rem;color:#4a5a7a;flex-shrink:0;overflow:hidden;border:1px solid rgba(255,255,255,0.04);" id="imagePreview">${imagePreviewContent}</div>
                    <div style="flex:1;">
                        <div style="font-size:0.8rem;color:#5a6388;" id="fileNameText">${isEdit && item.image_url ? 'Current image uploaded' : 'Click or drag to upload image'}</div>
            ...
            <div class="form-group">
                <label style="display:block;font-size:12px;color:#5a6388;margin-bottom:4px;font-weight:600;"><i class="fas fa-link"></i> CTA Link (点击跳转)</label>
                ...
                <div style="font-size:0.6rem;color:#2a3560;margin-top:4px;"><i class="fas fa-info-circle"></i> 留空则不跳转</div>
            </div>
            <div class="form-group">
        ...
        if (typeof openModal === 'function') {
            openModal('<i class="fas fa-' + (isEdit ? 'edit' : 'plus') + '"></i> ' + title, subtitle, bodyHTML, function() {
                if (selectedFile) {
                    ...
                    uploadImage(selectedFile)
                        .then(function(url) { uploadedImageUrl = url; saveFormData(); })
            ...
            setTimeout(function() {
                ...
                fileInput.addEventListener('change', function() {
                    ...
                    var reader = new FileReader();
                    reader.onload = function(e) { imagePreview.innerHTML = '<img src="' + e.target.result + '" style="width:100%;height:100%;object-fit:cover;border-radius:8px;">'; };
                    reader.readAsDataURL(file);
                });
    ...
    function saveFormData() {
        ...
        var data = { image_url: uploadedImageUrl, cta_link: cta_link, active: active };
